...
namespace server {
	...
	class TCPGameServer
	{
		...
		public List<GameRoom> GetGameRooms() { return _gameRooms; }
		...
		{
			_playerInfo.Add(pClient, playerInfo);
		}
		private void RemoveInactiveGames()
		{
			...
			for (int i = 0; i < _gameRooms.Count; i++)
			{
				if (!_gameRooms[i].IsGameInPlay && _gameRooms[i].MemberCount == 0)
				{
					inactiveRooms.Add(_gameRooms[i]);
				}
			}
			for (int i = 0; i < inactiveRooms.Count; i++)
			{
				_gameRooms.Remove(inactiveRooms[i]);
